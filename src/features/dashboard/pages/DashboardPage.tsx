...
const todayActivities = [
  { id: 'endpoint-pagos', title: 'Implementar endpoint de pagos', area: 'Trabajo', progress: '4/8', percent: 50, priority: 'Alta' },
  { id: 'pc-inmunologia', title: 'PC de Inmunologia', area: 'Universidad', progress: '3/6', percent: 50, priority: 'Urgente' },
  { id: 'gimnasio', title: 'Gimnasio', area: 'Personal', progress: '0/3', percent: 0, priority: 'Media' }
];
    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link className="flex h-10 w-10 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 shadow-sm" to="/activities" title="Buscar">
            <Search size={18} />
          </Link>
          <Link className="flex h-10 items-center gap-2 rounded-md bg-teal-700 px-4 text-sm font-semibold text-white shadow-sm" to="/activities/new">
            <Plus size={18} />
            Nueva
          </Link>
        </div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {summary.map((item) => (
          <Link className={`rounded-lg border p-4 transition hover:-translate-y-0.5 hover:shadow-sm ${item.tone}`} key={item.label} to="/activities">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
        <div>
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link className="flex items-center gap-1 text-sm font-medium text-teal-700" to="/activities">
              Ver todo
              <ArrowRight size={16} />
            </Link>
          </div>
          <div className="space-y-3">
            {todayActivities.map((activity) => (
              <Link className="block rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition hover:border-teal-200 hover:shadow-md" key={activity.title} to={`/activities/${activity.id}`}>
        ...
        <aside className="space-y-4">
          <Link className="block rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition hover:border-teal-200 hover:shadow-md" to="/calendar">
          ...
          <Link className="block rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition hover:border-amber-200 hover:shadow-md" to="/calendar">
            <div className="mb-3 flex items-center gap-2">
          ...
          <Link className="block rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition hover:border-emerald-200 hover:shadow-md" to="/activities">
            <div className="mb-3 flex items-center gap-2">
      ...
      <Link className="fixed bottom-20 right-4 flex h-14 w-14 items-center justify-center rounded-full bg-teal-700 text-white shadow-lg lg:hidden" to="/activities/new" title="Nueva actividad">
        <ListTodo size={22} />
      </Link>
    </div>
  );
